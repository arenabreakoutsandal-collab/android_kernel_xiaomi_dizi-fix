...
static char *kdump_reason[8] = { "Unknown",  "Kernel Panic", "Oops!",
				 "Emerg",    "Shut Down",    "Restart",
				 "PowerOff", "Long Press" };
...
static char *log_type[4] = { "Unknown", "LAST KMSG", "LAST LOGCAT" };

struct atomic_notifier_head pwrkey_irq_notifier_list;
...
module_param_string(fingerprint, build_fingerprint, MAX_CMDLINE_PARAM_LEN,
		    0644);
...
static void mtdoops_inc_counter(struct mtdoops_context *cxt)
{
	...
	printk(KERN_ERR
	       "mtdoops: ready nextpage= %d, nextcount= %d (no erase), oops_page_used_flag = %lx",
			    ...
			    enum mtd_dump_reason reason)
{
	...
	if (cxt->mtd == NULL) {
		printk(KERN_ERR
		       "mtdoops: init is not finish. Cannot write mtd logs \n");
		return;
	}

	do_dump_count++;
	printk(KERN_ERR
	       "mtdoops: %s start , count = %d , page = %d, reason = %d, dump_count = %d\n",
...
static int pwrkey_long_press_irq_event(struct notifier_block *this,
				       unsigned long event, void *ptr)
{
	mtdoops_do_dump_if(MTD_DUMP_LONG_PRESS);
	return NOTIFY_DONE;
}
...
static void mtdoops_notify_add(struct mtd_info *mtd)
{
	...
	atomic_notifier_chain_register(&pwrkey_irq_notifier_list,
...
#undef parse_u32
	...
	printk(KERN_ERR
	       "mtdoops: pares mtd_dt, mem_address =0x%x, mem_size =0x%x \n",
	       cxt->pmsg_data.mem_address, cxt->pmsg_data.mem_size);
...
static const struct of_device_id dt_match[] = { { .compatible =
							  "mtdoops_pmsg" },
						{} };
