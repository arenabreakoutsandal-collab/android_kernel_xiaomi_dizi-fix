...
#define MI_FP_3V2
static struct regulator *p_3v2_vreg = NULL;
...
static int enable_regulator_3V2(struct device *dev, struct regulator **pp_vreg)
{
	...
	// vreg = devm_regulator_get(dev, "pm8350c_l9");
	vreg = devm_regulator_get(dev, "l9c_vdd");
	if (IS_ERR(vreg)) {
...
#if 0
	rc = regulator_set_voltage(vreg, 3200000, 3200000);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_RESET:
		...
		if (gf_dev->pinctrl && gf_dev->gf_rst_state) {
			if (pinctrl_select_state(gf_dev->pinctrl,
						 gf_dev->gf_rst_state) < 0) {
